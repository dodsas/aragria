...
- **좌측 D-pad** — 4방향 이동.
- **우측 액션 패드** — `공격` / `봐` 계층 메뉴. 방의 실제 대상이 있을 때만 노출.
...
`actionPadLevel ∈ {'root', 'attack', 'look'}` 단일 상태 머신.

### Root

- `[공격]` — 방에 살아있는 몬스터가 1마리 이상일 때만 표시.
- `[봐]` — 방에 몬스터·오브젝트·다른 플레이어 중 하나라도 있으면 표시.
- 둘 다 없으면 빈 패드(아무 버튼도 노출되지 않음). 사용자가 의미 없는 메뉴를 탭하지 않게 하려는 의도.

탭하면 해당 서브메뉴로 진입하되 명령은 **이 시점에 보내지 않는다**.
...
서브메뉴에 머무는 도중 마지막 타깃이 사라지면(예: 몬스터를 다 잡음) 다음 `room_monsters` 갱신 시 `renderActionPad()` 가 빈 리스트를 감지하고 자동으로 root로 복귀한다. 빈 패널이 노출되는 상태를 만들지 않는다.
